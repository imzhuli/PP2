...
public:
	...
	void OnGeoQuery(const xPacketHeader & Header, ubyte * PayloadPtr, size_t PayloadSize) {
		...
		auto Resp      = xGeoInfoResp();
		Resp.IP        = AS;
		Resp.CountryId = GI.CountryId;
		Resp.CityId    = GI.CityId;
		ubyte  RespBuffer[MaxPacketSize];
		size_t RespSize = WritePacket(Cmd_GeoQueryResp, Header.RequestId, RespBuffer, sizeof(RespBuffer), Resp);
