...
static bool OnDownloadServerList(const xTcpServiceClientConnectionHandle & Handle, ubyte * PayloadPtr, size_t PayloadSize) {
    ...
    if (Steal(ServiceTypeInfo.Dirty)) {
        ...
        DEBUG_LOG("updated server list response buffer, ServiceType=%u, CurrentVersion=%" PRIu32 "", (unsigned)Request.ServiceType, ServiceTypeInfo.Version);
    } else {
        DEBUG_LOG("from response buffer, ServiceType=%u, CurrentVersion=%" PRIu32 "", (unsigned)Request.ServiceType, ServiceTypeInfo.Version);
    }
